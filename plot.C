#include <TLine.h>
#include <TH2F.h>
// define bin and line widths for the histogram
#define bwidth 1.1 // bin width for the histogram
#define lwidth 1.0 // line width for the histogram
...
void plot(float M_chi10=-9999,
		      ...
		      TString outdir="./")
{
  cout << "Genearting the Mass Spectrum..." << endl;

  SetStyle();

// Define color categories for the mass spectrum
  int color1 = kRed;
  int color2 = kBlue;
  int color3 = kGreen;
  int color4 = kGreen;
  int color5 = kCyan+1;
  int color6 = kMagenta+2;
  int color7 = kBlack;

    //Get Lines ans assign color category to them
    int N=0;
  ...
  TLatex* T_nu_tauL = GetLabel("#tilde{#nu}_{#tauL}", M_nu_tauL,  N+=1);
    ...
    int Nparticles=0;
    if( M_chi10   > 0 )       Nparticles++;
    if( M_chi20   > 0 )       Nparticles++;
    if( M_chi30   > 0 )       Nparticles++;
    if( M_chi40   > 0 )       Nparticles++;
    if( M_chi1p   > 0 )       Nparticles++;
    if( M_chi2p   > 0 )       Nparticles++;
    if( M_uL      > 0 )       Nparticles++;
    if( M_uR      > 0 )       Nparticles++;
    if( M_dL      > 0 )       Nparticles++;
    if( M_dR      > 0 )       Nparticles++;
    if( M_sL      > 0 )       Nparticles++;
    if( M_sR      > 0 )       Nparticles++;
    if( M_cL      > 0 )       Nparticles++;
    if( M_cR      > 0 )       Nparticles++;
    if( M_t1      > 0 )       Nparticles++;
    if( M_t2      > 0 )       Nparticles++;
    if( M_b1      > 0 )       Nparticles++;
    if( M_b2      > 0 )       Nparticles++;
    if( M_eL      > 0 )       Nparticles++;
    if( M_eR      > 0 )       Nparticles++;
    if( M_nu_eL   > 0 )       Nparticles++;
    if( M_muL     > 0 )       Nparticles++;
    if( M_muR     > 0 )       Nparticles++;
    if( M_nu_muL  > 0 )       Nparticles++;
    if( M_tau1    > 0 )       Nparticles++;
    if( M_tau2    > 0 )       Nparticles++;
    if( M_nu_tauL > 0 )       Nparticles++;
    if( M_h0      > 0 )       Nparticles++;
    if( M_H       > 0 )       Nparticles++;
    if( M_Hp      > 0 )       Nparticles++;
    if( M_A       > 0 )       Nparticles++;
    if( M_gl      > 0 )       Nparticles++;
    if( M_G       > 0 )       Nparticles++;
...
// Draw mass spectrum line if it is non-zero value
